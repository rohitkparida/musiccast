(function(document) {
    ...
    function prevTrack() {
        PlayList.pos--;
        if (PlayList.pos <= -1) {
            PlayList.pos = Playlist.names.length - 1
        };
        rebuildPlayList();
        tryPlay();
    }
    ...
    function nextTrack() {
        PlayList.pos++;
        if (PlayList.pos >= PlayList.names.length) {
            PlayList.pos = 0
        };
        rebuildPlayList();
        tryPlay();
        ...
        if (PlayList.names.length > 1) {
            for (var i = 0; i < PlayList.names.length; i++) {
                ...
                if (i == PlayList.selected) {
                    PlayList.selected = randomTrack
                } else if (randomTrack == PlayList.selected) {
                    PlayList.selected = i
                }
                if (i == PlayList.pos) {
                    PlayList.pos = randomTrack
                } else if (randomTrack == PlayList.pos) {
                    PlayList.pos = i
                }
            }
        }
    ...
    function rebuildPlayList() {
        ...
        if (PlayList.names.length > 0) {
            for (var i = 0; i < PlayList.names.length; i++) {
                var spanClass = "song";
                if (i == PlayList.selected) {
                    spanClass = "selectedSong"
                }
                if (i == PlayList.pos) {
                    spanClass = "currentSong"
                }
                if ((i == PlayList.pos) && (i == PlayList.selected)) {
                    spanClass = "currentAndSelectedSong"
                }
            ...
            for (var i = 0; i < PlayList.names.length; i++) {
                ...
                eventBuffer.addEventListener("click", function() {
                    playlistClick((this.id))
                });
                eventBuffer.addEventListener("dblclick", function() {
                    playlistDoubleClick(this.id)
                });
    ...
    function trackDown() {
        if (swapTrack(PlayList.selected, PlayList.selected + 1)) {
            ...
            if (PlayList.selected < PlayList.names.length - 1) {
                PlayList.selected++;
            }
            rebuildPlayList();
        }
